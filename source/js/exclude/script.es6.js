...
function toggleOptionSelects() {
  $('.js-opt-select-parent').each(function () {
    $(this).find('.js-opt-select-coll').each(function () {
      const select = $(this).find('.js-opt-select')
      const allOptions = select[0].options
      const button = $(this).find('.js-opt-select-btn')
      const siblingSelect = $(this).siblings().first().find('.js-opt-select')
...
function initAutoHeightTextarea() {
  $('.js-textarea-auto').each(function () {
    const scroll = $(this).prop('scrollHeight')
